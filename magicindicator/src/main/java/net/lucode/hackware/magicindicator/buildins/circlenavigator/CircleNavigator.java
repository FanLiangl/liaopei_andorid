...
public class CircleNavigator extends View implements IPagerNavigator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint2.setColor(mCircleSelectColor);
        drawCircles(canvas);
    ...
    private void drawCircles(Canvas canvas) {
        ...
        for (int i = 0, j = mCirclePoints.size(); i < j; i++) {
            ...
            canvas.drawCircle(pointF.x, pointF.y, mRadius, mPaint2);
        }
    }
    ...
    public void onPageSelected(int position) {
        mCurrentIndex = position >= mCirclePoints.size() ? mCirclePoints.size()-1 : position;
        if (!mFollowTouch) {
            mIndicatorX = mCirclePoints.get(mCurrentIndex).x;
            invalidate();
        }
    ...
    public void setCircleSelectColor(int circleSelectColor){
        mCircleSelectColor = circleSelectColor;
        invalidate();
    }
