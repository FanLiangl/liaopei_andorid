...
public class LinePagerIndicator extends View implements IPagerIndicator {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (isGradient) {
            int startColor;
            int endColor;
            if (mColors != null && mColors.size() >= 2) {
                startColor = mColors.get(0);
                endColor = mColors.get(1);
            } else {
                startColor = Color.RED;
                endColor = Color.RED;
            }
            LinearGradient linearGradient = new LinearGradient(mLineRect.left, mLineRect.top, mLineRect.right, mLineRect.bottom,
                    new int[]{startColor, endColor}, null, Shader.TileMode.REPEAT);
